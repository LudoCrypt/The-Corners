...
public class CornerWorld {
	...
	public static final LiminalWorld YEARNING_CANAL = get("yearning_canal", new LiminalWorld(TheCorners.id("yearning_canal"), DimensionType.create(OptionalLong.of(1200), true, false, false, true, 1.0, false, false, true, false, false, 0, CornerConfig.getInstance().condensedDimensions ? 432 : 2032, CornerConfig.getInstance().condensedDimensions ? 432 : 2032, TheCorners.id("yearning_canal"), TheCorners.id("yearning_canal"), 1.0F), (biomeRegistry, seed) -> new YearningCanalChunkGenerator(new FixedBiomeSource(biomeRegistry.getOrThrow(CornerWorld.YEARNING_CANAL_BIOME)), seed), () -> new YearningCanalSky()));
	public static final LiminalWorld COMMUNAL_CORRIDORS = get("communal_corridors", new LiminalWorld(TheCorners.id("communal_corridors"), DimensionType.create(OptionalLong.of(23500), true, false, false, true, 1.0, false, false, true, false, false, 0, 128, 128, TheCorners.id("communal_corridors"), TheCorners.id("communal_corridors"), 0.075F), (biomeRegistry, seed) -> new CommunalCorridorsChunkGenerator(new FixedBiomeSource(biomeRegistry.getOrThrow(CornerWorld.COMMUNAL_CORRIDORS_BIOME)), seed), () -> new CommunalCorridorsSky()));
